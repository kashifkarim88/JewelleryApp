...
// Existing database products for validation and reference
...
export default function StockPage() {
    ...
    const codeInputRef = useRef<HTMLInputElement>(null); // Ref for unique code input
    ...
    useEffect(() => {
        const handleOutsideClick = (e: MouseEvent) => {
            ...
            if (prodRef.current && !prodRef.current.contains(e.target as Node)) setIsProdOpen(false);
        };
        ...
        <div className="min-h-screen bg-[#F8FAFC] p-4 md:p-8 text-slate-900 antialiased font-sans">
            <div className="max-w-6xl mx-auto relative">
                ...
                <header className="mb-8">
                    ...
                    <p className='text-slate-500 text-sm'>Add new unique pieces with real-time validation.</p>
                </header>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                    <div className="lg:col-span-8 space-y-6 bg-white rounded-3xl border border-slate-200 p-6 shadow-sm">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                        ...
                                        <div className="absolute z-50 w-full mt-2 bg-white border border-slate-100 rounded-2xl shadow-2xl max-h-48 overflow-y-auto p-2">
                                            {CATEGORIES.filter(c => c.toLowerCase().includes(catSearch.toLowerCase())).map(cat => (
                                                <div key={cat} onMouseDown={() => { setSelectedCat(cat); setCatSearch(cat); setIsCatOpen(false); }}
                                                    className="px-4 py-2.5 hover:bg-blue-50 rounded-xl cursor-pointer text-sm font-medium flex justify-between items-center">
                            ...
                            {/* Product Name (Dropdown showing Code) */}
                                            ...
                                            {PRODUCTS
                                                ...
                                                .map(prod => (
                                                    <div key={prod.id} onMouseDown={() => {
                                                        ...
                                                        setProdCode(generatePrefix(prod.name));
                                                        setIsProdOpen(false);
                                                        // Auto-focus the unique code input after a slight delay
                                                        setTimeout(() => codeInputRef.current?.focus(), 100);
                                                    }}
                                                        className="px-4 py-2.5 hover:bg-blue-50 rounded-xl cursor-pointer text-sm font-medium flex justify-between items-center group transition-colors">
                                                        <span>{prod.name}</span>
                                                        <span className="text-[10px] bg-slate-100 text-slate-500 px-2 py-0.5 rounded-md font-mono group-hover:bg-blue-100 group-hover:text-blue-600">
                                                            {prod.code}
                                                        </span>
                                                    </div>
                                                ))
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 pt-4">
                            <div>
                                ...
                                <input ref={workerRef} type="text" placeholder="Worker Name" className="w-full px-4 py-3 bg-slate-50 border border-slate-100 rounded-xl outline-none text-sm focus:bg-white focus:border-blue-500" value={vals.worker} onChange={(e) => setVals({ ...vals, worker: e.target.value })} />
                            </div>
                            <div>
                        ...
                        <div className="space-y-3 pt-6 border-t border-slate-50">
                            {[
                                ...
                                <div key={i} className={`rounded-2xl border transition-all ${sec.show ? 'border-blue-200 bg-blue-50/20 shadow-sm' : 'border-slate-100 bg-white'}`}>
                                    ...
                                    {sec.show && (
                                        <div className="p-4 pt-0 grid grid-cols-1 md:grid-cols-2 gap-4 animate-in fade-in duration-200">
                                            <div className="grid grid-cols-2 gap-2">
                    ...
                    <div className="lg:col-span-4 sticky top-8">
                        <div className="bg-white rounded-3xl border border-slate-200 p-6 shadow-sm">
                            <div className="aspect-square w-full relative rounded-2xl border-2 border-dashed border-slate-100 bg-slate-50 overflow-hidden flex flex-col items-center justify-center group hover:border-blue-300 transition-all mb-6">
                                {imagePreview ? (
                                    <>
                                        ...
                                        <button onClick={() => setImagePreview(null)} className="absolute top-3 right-3 p-2 bg-white/90 rounded-xl shadow-md text-red-500 hover:bg-red-50 transition-colors"><X size={16} /></button>
                                    </>
                                ) : (
                            ...
                            <button
                                ...
                                className="w-full bg-slate-900 hover:bg-black text-white font-bold py-4 rounded-2xl transition-all active:scale-[0.98] disabled:bg-slate-50 disabled:text-slate-300 flex items-center justify-center gap-2 shadow-xl shadow-slate-200"
                            >
